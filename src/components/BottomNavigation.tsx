...
const BottomNavigation: React.FC = () => {
  ...
  return (
    <nav
      className="w-full max-w-[428px] h-16 bg-white border-t border-gray-200 shadow-[0_-4px_12px_rgba(0,0,0,0.08)] flex-shrink-0">
      <div className="flex h-full">
        {tabs.map((tab) => (
          <NavLink
            ...
            to={tab.path}
            className={({isActive}) =>
              `flex-1 flex flex-col items-center justify-center px-2 transition-all duration-200 relative
                ${isActive
                ? "text-primary-600"
